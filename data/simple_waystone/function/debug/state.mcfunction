...
execute store result score #debug.count sws.tmp if entity @e[type=minecraft:interaction,tag=sws.waystone,tag=sws.clickable]
...
execute store result score #debug.label sws.tmp if entity @e[type=minecraft:armor_stand,tag=sws.waystone,tag=sws.label]
tellraw @s [{"text":"- loaded name labels present: ","color":"gray"},{"score":{"name":"#debug.label","objective":"sws.tmp"},"color":"yellow"}]
