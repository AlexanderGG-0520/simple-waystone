# Advancement reward for using a Waystone Core on a block.
# The core item is a custom_data-marked lodestone and counts as the lodestone part of the cost.
...
execute if score #core.ender_eye sws.tmp matches 16.. if score #core.diamond_block sws.tmp matches 4.. run scoreboard players set @s sws.has_cost 1
...
execute unless score @s sws.has_cost matches 1 run tellraw @s [{"text":"[Simple Waystone] ","color":"gold"},{"text":"Waystone Core needs the remaining cost: 16 ender eyes and 4 diamond blocks.","color":"red"}]
...
execute if score @s sws.tmp matches 1 store result score #core.consumed sws.tmp run clear @s minecraft:lodestone[minecraft:custom_data~{simple_waystone:{core:1b}}] 1
