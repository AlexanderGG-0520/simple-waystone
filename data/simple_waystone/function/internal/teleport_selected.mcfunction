...
execute unless entity @e[type=minecraft:interaction,tag=sws.waystone,tag=sws.clickable,tag=sws.destination,limit=1] run tellraw @s [{"text":"[Simple Waystone] ","color":"gold"},{"text":"That destination no longer exists.","color":"red"}]
